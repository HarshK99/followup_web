...
export const createVisitAPI = async (visit: any) => {
  return apiClient.post('/api/visits', visit);
};

export const createFollowupAPI = async (followup: any) => {
  return apiClient.post('/api/follow-ups', followup);
};

export const getSalesFollowupsAPI = async () => {
  return apiClient.get('/api/follow-ups?scope=mine');
};

export const getManagerFollowupsAPI = async (filter: string) => {
  return apiClient.get(`/api/follow-ups?scope=all&filter=${filter}`);
};
